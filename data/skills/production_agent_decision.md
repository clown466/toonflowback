...
- **小说保真约束**：当用户说“按我的小说走 / 根据小说 / 不要原创 / 直接塑角造景 / 直接 Seedance 分镜”时，生产 Agent 必须把小说章节原文、事件分析和项目资产库作为唯一剧情来源，直接推进制作流水线；不得要求原创模式参数，不得创作新剧情，不得改写原著主线。缺少前置内容时，只提示需要先完成“小说事件分析/资产提取”中的具体缺项。

---

## 项目级小说资产流程转交

如果用户请求的是整个项目的小说资产提取、塑角造景或资产库准备，应转交/建议使用 workspaceAgent。内部 `scriptId/episodesId` 只是章节分镜工作区键，不得对用户称为剧本或生产容器。
...
| 项 | 说明 |
|----|------|
| 派发 | 执行层分析小说章节/分镜上下文，识别并写入衍生资产信息 |
| 输出 | 衍生资产分析报告 + 衍生资产写入结果（或"无需衍生"结论） |
| 前置条件 | 小说章节/事件分析和资产已存在于工作区 |
| 审核 | 不需要 |
...
| 模型信息 | 决策层操作 |
|---------|-----------|
| 视频模型包含 `Seedance` | 无需询问用户，直接以 **"Seedance 分镜模式"** 派发给执行层 |
| 模型参数 `多参` = 是，且非 Seedance | 向用户询问：使用 **"纯文本多参模式"** 还是 **"分镜图辅助多参模式"**，等待用户确认后，将所选模式随任务指令一起派发给执行层 |
| 模型参数 `多参` = 否，且非 Seedance | 无需询问用户，直接以 **"首位帧模式"** 派发给执行层 |
...
- 分组累计时长不得超过 15 秒
- 派发执行层时必须在指令中明确携带写入模式（Seedance 分镜模式 / 纯文本多参模式 / 分镜图辅助多参模式 / 首位帧模式）
...
| "按我的小说走" / "根据小说" / "不要原创" / "直接塑角造景" / "直接 Seedance 分镜" | 基于小说章节、事件分析和项目资产库继续制作；若资产已存在，直接从阶段1衍生资产分析进入，再到导演规划、分镜表、Seedance 分镜面板；不得切原创模式 |
