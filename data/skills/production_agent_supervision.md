...
4. **叙事模式选型合理**：须根据内容特征选择叙事模式（完整叙事型/情绪意境型/原著保真型），选型须与章节/事件结构匹配
...
1. **内容忠实章节/事件**：分镜内容严格按照小说章节/事件分析的叙事顺序和内容拆分，不遗漏不新增源内容中不存在的情节
2. **台词原文锁定**：章节原文/事件记录中所有台词必须原文照搬进 lines 字段，禁止改写、省略或意译；缺失台词视为严重错误
...
2. 调用 `get_flowData` 获取章节内容数据（script）和资产数据（assets）
...
| 剧情覆盖度 | ③叙事结构 + ④分场景意图 | 段落划分与分场景意图覆盖源内容全部场次/关键事件，无遗漏（→ Skills 三-3） | 严重 |
...
| 章节/事件覆盖度 | 源内容中的全部场景和关键事件均有对应分镜，无遗漏（→ Skills 二-1/三-3） | 严重 |
| 台词完整性 | 源内容中所有台词均原文出现在分镜 lines 字段中，无改写、省略或遗漏（→ Skills 二-2/三-5） | 严重 |
...
| 时长合理性 | 含台词时长匹配情绪语速；无台词镜头≤6s；总时长与源内容预估基本吻合（→ Skills 二-9/11） | 中等 |
...
3. 标注缺失、改写或省略的台词及对应源内容位置

不通过示例：
- 原文写"你以为你配？"，分镜 lines 改写为"你觉得你配吗？"
- 源内容中某角色有 3 句台词，分镜表仅收录 2 句
