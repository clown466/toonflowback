...
description: >-
  ...
  负责分析小说章节/事件内容并识别每个资产的视觉状态变体，逐条写入衍生资产。
---
...
| 操作 | 调用 |
|------|------|
| 读取章节内容与资产 | `get_flowData("script")` / `get_flowData("assets")` |
| 写入衍生资产 | `add_deriveAsset` |


### 执行流程

1. 获取 `script`（章节内容/事件分析/章节工作区内容）和 `assets`
2. 按下方提取规则分析章节内容，识别每个资产的视觉状态变体
...
> **角色默认基准态**：角色父资产默认视为基础打底态（如白色背心+内裤，或仅内裤状态）。因此章节内容中凡出现校服、常服、礼服、盔甲、外套等明确穿着时，通常应补充对应的**服装类衍生资产**；基础打底态本身不重复创建衍生。若某角色当前**没有任何子资产**，则应先根据章节主场景与常态设定，补充一套最合适的**默认常服/正式服装衍生**，避免后续该角色长期停留在基础打底态。
...
- **常见误判原因**：把"章节重点描写"误当成"需要衍生资产"。判断标准不是它是否重要，而是它是否属于父资产**稳定、可复用、整体级**的视觉状态
- 若角色当前 `derive` 为空，应先补 1 个最符合章节常态的服装类衍生资产（如常服、校服、工装、礼服中的一种），作为后续主要出镜默认态
...
- 来源优先级：章节原文/事件分析明确描写 > 资产描述暗示 > 合理推测
- `name`：2~6 字，体现视觉外观变化
