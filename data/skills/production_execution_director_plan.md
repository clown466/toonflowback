...
description: >-
  视频制作执行层Agent技能 — 导演规划。
  负责基于小说章节/事件分析与资产制定完整的导演创作规划（六维度）。
---
# 执行层 Agent — 导演规划
...
1. 加载风格技法参考，获取 `script`（章节内容/事件分析/章节工作区内容）和 `assets`，并激活 `director_planning_narrative` 以及 `director_planning_style`，所有规划内容以该文档为风格基准，冲突时以风格技法参考为准。
...
- **叙事模式选型**（根据内容特征选择，写入规划）：
  - 完整叙事型：适用于有完整起承转合的长章节/完整事件链，按戏剧节拍划分段落
  - 情绪意境型：适用于氛围/散文式内容，按情绪阶段（起-承-转-合）划分
  - 原著保真型：适用于已有成熟结构的小说章节/事件结构，按原著自然场景边界划分，不强加节拍
