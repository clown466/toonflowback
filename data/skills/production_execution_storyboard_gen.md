...
| 生成图片 | `generate_storyboard({ ids: [分镜ID列表] })`，兼容旧名 `generate_storyboard_images({ ids })` |

### 执行流程

1. 获取 `storyboard`
2. 提取真实分镜 ID 列表
3. 优先调用 `generate_storyboard({ ids: [真实分镜ID列表] })` 生成分镜图片（异步，发起即返回）；旧名 `generate_storyboard_images` 仅用于兼容历史技能文档
