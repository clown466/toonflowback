...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  if (imageBase64List.length && referenceTransport === "publicUrlForm") {
    ...
    logger(`[custom2 imageRequest] POST /images/edits baseUrl=${baseUrl} model=${model.modelName} size=${resolvedSize} quality=${shouldSendQuality ? resolvedQuality : "default"} refs=${imageUrls.length} transport=publicUrlForm`);
