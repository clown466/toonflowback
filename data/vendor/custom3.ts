...
const vendor: VendorConfig = {
  ...
  author: "Toonflow",
  name: "自定义3",
  description: "## 自定义3 模型服务模板，OpenAI 兼容接口，可自行配置请求地址和模型。",
  inputs: [
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  const parseResponse = async (response: any, endpoint: string): Promise<string> => {
    ...
    if (!response.ok) {
      throw new Error(`custom3图片请求失败，接口: ${endpoint}, 状态码: ${response.status}, 错误信息: ${responseText}`);
    }
    let data: any;
    try {
      data = JSON.parse(responseText);
    } catch {
      throw new Error(`custom3图片响应不是JSON: ${responseText.slice(0, 500)}`);
    }
    const raw = pickImageResult(data);
    if (!raw || typeof raw !== "string") {
      throw new Error(`custom3图片响应中未找到图片URL/base64: ${JSON.stringify(data).slice(0, 800)}`);
    }
  ...
  if (imageBase64List.length) {
    ...
    logger(`[custom3 imageRequest] POST /images/edits baseUrl=${baseUrl} model=${model.modelName} size=${resolvedSize} quality=${/^gpt-image/i.test(model.modelName) ? "high" : "default"} refs=${imageBase64List.length}`);
  ...
  logger(`[custom3 imageRequest] POST /images/generations baseUrl=${baseUrl} model=${model.modelName} size=${resolvedSize} quality=${body.quality || "default"} refs=0`);
