...
const vendor: VendorConfig = {
  ...
  author: "Toonflow",
  name: "timeai",
  description: "## timeai 模型服务模板，基于 OpenAI 兼容接口，可在界面中配置 API Key 和请求地址。",
  inputs: [
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  // OpenAI-compatible image-generation endpoint. Do not pass response_format: timeai rejects it for gpt-image-*.
  ...
  if (imageBase64List.length) {
    // Some compatible providers accept image/image_url for reference generation; harmlessly included only when present.
    body.image = imageBase64List.length === 1 ? imageBase64List[0] : imageBase64List;
  }

  logger(`[timeai imageRequest] POST /images/generations model=${model.modelName} size=${resolvedSize} refs=${imageBase64List.length}`);
  ...
  if (!response.ok) {
    throw new Error(`timeai图片请求失败，状态码: ${response.status}, 错误信息: ${responseText}`);
  }
  ...
  try {
    ...
    throw new Error(`timeai图片响应不是JSON: ${responseText.slice(0, 500)}`);
  }
  ...
  if (!raw || typeof raw !== "string") {
    throw new Error(`timeai图片响应中未找到图片URL/base64: ${JSON.stringify(data).slice(0, 800)}`);
  }
