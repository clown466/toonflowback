...
export async function runDecisionAI(ctx: AgentContext) {
  ...
  const prompt = getSkillContentForAgent(await fs.promises.readFile(skill, "utf-8"), "scriptAgent:decisionAgent");
...
function createSubAgent(parentCtx: AgentContext) {
  ...
  const run_sub_agent_storySkeleton = tool({
    ...
    execute: async ({ prompt }) => {
      ...
      const systemPrompt = getSkillContentForAgent(await fs.promises.readFile(skill, "utf-8"), "scriptAgent:storySkeletonAgent");
  ...
  const run_sub_agent_adaptationStrategy = tool({
    ...
    execute: async ({ prompt }) => {
      ...
      const systemPrompt = getSkillContentForAgent(await fs.promises.readFile(skill, "utf-8"), "scriptAgent:adaptationStrategyAgent");

      const formatPrompt = "\n你必须使用如下XML格式写入工作区：\n<adaptationStrategy>创作策略/改编策略内容</adaptationStrategy>";
  ...
  const run_sub_agent_script = tool({
    ...
    execute: async ({ prompt }) => {
      ...
      const systemPrompt = getSkillContentForAgent(await fs.promises.readFile(skill, "utf-8"), "scriptAgent:scriptAgent");
  ...
  const run_supervision_agent = tool({
    ...
    execute: async ({ prompt }) => {
      ...
      const systemPrompt = getSkillContentForAgent(await fs.promises.readFile(skill, "utf-8"), "scriptAgent:supervisionAgent");
