...
function detectIntent(text: string, snapshot?: WorkspaceCommandSnapshot): IntentSignal | null {
  ...
  const modifyAssetImageIntent =
    /(修改|改成|改为|重绘|替换|重新设计|全新|从零设计|新形象|新造型).{0,64}(资产|角色|人物|场景|道具|参考图|图片|图像|角色图|场景图|道具图)|(资产|角色|人物|场景|道具|参考图|图片|图像|角色图|场景图|道具图).{0,64}(修改|改成|改为|重绘|替换|重新设计|全新|从零设计|新形象|新造型)/i.test(text);
