...
async function repairMissingArtifacts(knex: Knex): Promise<void> {
  ...
  for (const config of configs) {
    ...
    const rows = await db(config.table).where("state", config.successState).select("id", "filePath");
    const dirtyIds: number[] = [];

    for (const row of rows as Array<{ id: number; filePath?: string | null }>) {
      const filePath = typeof row.filePath === "string" ? row.filePath.trim() : "";
      if (!filePath || !(await u.oss.isValidFile(filePath, 1))) dirtyIds.push(row.id);
    }

    if (dirtyIds.length > 0) {
      await db(config.table).whereIn("id", dirtyIds).update({
        state: config.failedState,
        [config.reasonColumn]: MISSING_ARTIFACT_REASON,
      });
    }
...
export default async (knex: Knex): Promise<void> => {
  ...
  const getVendorVersion = (vendorId: string): number | null => {
    try {
      ...
      console.warn(`[fixDB] vendor ${vendorId} 加载失败，跳过版本迁移：${u.error(error).message}`);
      return null;
    }
  };

  const volcengineVer = getVendorVersion("volcengine");
  if (volcengineVer !== null && volcengineVer < 2.3 && vendorData["volcengine.ts"]) {
    u.vendor.writeCode("volcengine", vendorData["volcengine.ts"]);
  }
  const minimaxVer = getVendorVersion("minimax");
  if (minimaxVer !== null && minimaxVer < 2.1 && vendorData["minimax.ts"]) {
