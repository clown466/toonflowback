...
export default router.post(
  ...
  async (req, res) => {
    ...
    const generateSingleAsset = async (item: any) => {
      ...
      let text = item.prompt || item.describe || item.name || "";
      try {
        const promptResult = await u.Ai.Text("universalAi").invoke({
        ...
        text = promptResult.text || text;
        await u.db("o_assets").where("id", item.id).update({ prompt: text });
      } catch (e) {
        console.error("[batchGenerateAssetsImage] 资产提示词生成失败，改用已有 prompt/describe", { assetId: item.id, error: u.error(e).message });
        await u.db("o_assets").where("id", item.id).update({ promptErrorReason: u.error(e).message });
      }
      ...
      try {
        ...
        const updated = await u.db("o_image").where({ id: imageId, state: "生成中" }).update({ state: "已完成", filePath: savePath });
        if (updated === 0) {
          return {
            id: item.id!,
            state: "已取消或已被新任务替换",
        ...
        const updated = await u
          .db("o_image")
          .where({ id: imageId, state: "生成中" })
          .update({ state: "生成失败", errorReason: u.error(e).message });
        return {
          id: item.id!,
          state: updated === 0 ? "已取消或已被新任务替换" : "生成失败",
          src: "",
        };
      }
    };
