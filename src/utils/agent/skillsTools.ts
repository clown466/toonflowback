...
export type ToonflowAgentKey = `${"productionAgent" | "scriptAgent"}:${string}` | string;

interface ToonflowSkillSection {
  target: string;
  priority: number;
  content: string;
}
...
function parseToonflowAttrs(rawAttrs: string): Record<string, string> {
...
function agentAliases(agentKey: ToonflowAgentKey): string[] {
  const [rawGroup, rawName = ""] = String(agentKey).split(":", 2);
  const group = rawGroup === "productionAgent" ? "production" : rawGroup === "scriptAgent" ? "script" : rawGroup;
  return [String(agentKey), `${group}:${rawName}`, `${rawGroup}:*`, `${group}:*`, "*"];
}

function sectionTargetsAgent(sectionTarget: string, agentKey: ToonflowAgentKey): boolean {
  const targets = sectionTarget
    .split(/[\s,|]+/)
    .map((t) => t.trim())
    .filter(Boolean);
  if (!targets.length) return true;
  const aliases = agentAliases(agentKey);
  return targets.some((target) => aliases.includes(target));
}

export function parseToonflowSkillSections(content: string): { hasSections: boolean; sections: ToonflowSkillSection[]; legacyBody: string } {
  const legacyBody = stripFrontmatter(content);
  const sections: ToonflowSkillSection[] = [];
  const sectionRegex = /<toonflow-skill\b([^>]*)>([\s\S]*?)<\/toonflow-skill>/gi;
  let match: RegExpExecArray | null;
  while ((match = sectionRegex.exec(legacyBody)) !== null) {
    const attrs = parseToonflowAttrs(match[1] ?? "");
    const body = (match[2] ?? "").trim();
    if (!body) continue;
    sections.push({
      target: attrs.target || "*",
      priority: Number.isFinite(Number(attrs.priority)) ? Number(attrs.priority) : 100,
      content: body,
    });
  }
  return { hasSections: sections.length > 0, sections, legacyBody };
}

export function getSkillContentForAgent(content: string, agentKey: ToonflowAgentKey = "*"): string {
  const parsed = parseToonflowSkillSections(content);
  if (!parsed.hasSections) return ensureNonEmptyBody(parsed.legacyBody, "该技能文件无正文内容。");
  const matched = parsed.sections
    .filter((section) => sectionTargetsAgent(section.target, agentKey))
    .sort((a, b) => a.priority - b.priority)
    .map((section) => section.content)
    .join("\n\n");
  return ensureNonEmptyBody(matched, "该技能文件没有匹配当前 Agent 的分区指令。");
}
  ...
  return {
    activate_skill: tool({
      ...
      execute: async ({ name }) => {
        ...
        const body = getSkillContentForAgent(raw, agentKey);
        let content = "";
