...
type GeneratedKind = "image" | "video" | "audio";

function generatedKindLabel(kind: GeneratedKind) {
  return kind === "image" ? "图片" : kind === "video" ? "视频" : "音频";
}

function minGeneratedBytes(kind: GeneratedKind) {
  return kind === "image" ? MIN_IMAGE_BYTES : kind === "video" ? MIN_VIDEO_BYTES : MIN_AUDIO_BYTES;
}

function assertNonEmptyGeneratedResult(kind: GeneratedKind, result: string) {
  if (typeof result !== "string" || result.trim().length === 0) {
    throw new Error(`供应商未返回有效${generatedKindLabel(kind)}内容`);
  }
}

function assertGeneratedBuffer(kind: GeneratedKind, buffer: Buffer) {
  if (!Buffer.isBuffer(buffer) || buffer.length < minGeneratedBytes(kind)) {
    throw new Error(`供应商返回的${generatedKindLabel(kind)}内容为空或过小`);
  }
}

function decodeGeneratedBase64(kind: GeneratedKind, result: string): Buffer {
  assertNonEmptyGeneratedResult(kind, result);
  const base64 = result.trim().replace(/^data:[^;]+;base64,/, "").replace(/\s/g, "");
  if (!base64 || base64.length % 4 === 1 || !/^[A-Za-z0-9+/]+={0,2}$/.test(base64)) {
    throw new Error(`供应商返回的${generatedKindLabel(kind)}内容不是有效base64`);
  }
  const buffer = Buffer.from(base64, "base64");
  assertGeneratedBuffer(kind, buffer);
  return buffer;
}

async function assertValidGeneratedArtifact(kind: GeneratedKind, buffer: Buffer) {
  assertGeneratedBuffer(kind, buffer);
  if (kind === "image") {
    try {
      await sharp(buffer).metadata();
    } catch {
      throw new Error("供应商返回的图片内容无效");
    }
  }
}

async function urlToBase64(url: string, kind: GeneratedKind, retries = 3, delay = 1000): Promise<string> {
...
function referenceList2imageBase642(id: string, input: any) {
  let version: unknown;
  try {
    version = u.vendor.getVendor(id)?.version;
  } catch (error) {
    console.warn(`[ai] vendor ${id} 加载失败，按旧版 referenceList 兼容处理：${u.error(error).message}`);
  }
  const versionNumber = typeof version === "string" || typeof version === "number" ? parseFloat(String(version)) : Number.NaN;
  if (!version || isNaN(versionNumber) || versionNumber < 2.0) {
...
class AiImage {
  ...
  async run(input: ImageConfig, taskRecord?: TaskRecord) {
    ...
    const exec = async (mn: `${string}:${string}`) => {
      ...
      assertNonEmptyGeneratedResult("image", this.result);
      if (this.result.startsWith("http")) this.result = await urlToBase64(this.result, "image");
      assertNonEmptyGeneratedResult("image", this.result);
      return this;
    };
    if (taskRecord) {
  ...
  async save(path: string) {
    const buffer = decodeGeneratedBase64("image", this.result);
    await assertValidGeneratedArtifact("image", buffer);
    await u.oss.writeFile(path, buffer);
    await assertValidGeneratedArtifact("image", await u.oss.getFile(path));
    return this;
  }
}
...
class AiVideo {
  ...
  async run(input: VideoConfig, taskRecord?: TaskRecord) {
    ...
    const exec = async (mn: `${string}:${string}`) => {
      ...
      this.result = await fn(input);
      assertNonEmptyGeneratedResult("video", this.result);
      if (this.result.startsWith("http")) this.result = await urlToBase64(this.result, "video");
      assertNonEmptyGeneratedResult("video", this.result);
      return this;
    };
    if (taskRecord) {
  ...
  async save(path: string) {
    const buffer = decodeGeneratedBase64("video", this.result);
    await assertValidGeneratedArtifact("video", buffer);
    await u.oss.writeFile(path, buffer);
    await assertValidGeneratedArtifact("video", await u.oss.getFile(path));
    return this;
  }
}
class AiAudio {
  ...
  async run(input: VideoConfig, taskRecord?: TaskRecord) {
    ...
    const exec = async (mn: `${string}:${string}`) => {
      ...
      await referenceList2imageBase642(mn.split(/:(.+)/)[0], input);
      this.result = await fn(input);
      assertNonEmptyGeneratedResult("audio", this.result);
      if (this.result.startsWith("http")) this.result = await urlToBase64(this.result, "audio");
      assertNonEmptyGeneratedResult("audio", this.result);
      return this;
    };
    if (taskRecord) {
  ...
  async save(path: string) {
    const buffer = decodeGeneratedBase64("audio", this.result);
    await assertValidGeneratedArtifact("audio", buffer);
    await u.oss.writeFile(path, buffer);
    await assertValidGeneratedArtifact("audio", await u.oss.getFile(path));
    return this;
  }
}
