...
class OSS {
   ...
   * 获取 OSS 相对路径对应文件的状态。
   * @param userRelPath 用户传入的相对文件路径（使用 / 作为分隔符）
   * @returns 文件存在且为普通文件时返回 stat，否则返回 null
   */
  async statFile(userRelPath: string): Promise<{ size: number } | null> {
    await this.ensureInit();
    if (!userRelPath || typeof userRelPath !== "string") return null;
    try {
      const stat = await fs.stat(resolveSafeLocalPath(userRelPath, this.rootDir));
  ...
  /**
   * 检查 OSS 相对路径是否为存在且大于等于指定字节数的普通文件。
   * @param userRelPath 用户传入的相对文件路径（使用 / 作为分隔符）
   * @param minBytes 最小字节数，默认 1；传 1 可排除 0 字节文件
   */
  async isValidFile(userRelPath: string, minBytes: number = 1): Promise<boolean> {
    const stat = await this.statFile(userRelPath);
